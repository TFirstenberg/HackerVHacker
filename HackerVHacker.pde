...
class Goal
{
  ...
  private int xPos;
  private int yPos;
  private boolean hasBeenReached; //turns true when one player's path reaches it
  ...
  //methods
  public void onBlockColision(boolean isGoodPath, String winner)
  {
    if(isGoodPath == true)
    {
      //insert the stuff needed to end the game here
      //include a message that says that winner won
    }
  }
}